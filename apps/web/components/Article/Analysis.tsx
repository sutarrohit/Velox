...
import { RateLimitError } from "@/lib/apis/chat/chat-apis";
...
const Analysis = ({ article }: AnalysisProps) => {
  if (!article) {
    return (
      <div className="flex flex-1 w-full h-full items-center justify-center border  ">
        Article not found
      </div>
    );
  }
  ...
  const { data, isLoading, isError, error } = useQuery(
    summarizeArticleOptions(article),
  );
  ...
  const sentiment =
    sentimentConfig[data?.sentiment as keyof typeof sentimentConfig] ||
  ...
  return (
    <Card className="relative mx-auto w-full pt-0 h-full border">
      <CardHeader className="font-mono p-0 ">
        <CardTitle className="flex gap-2 flex-wrap items-center text-chart-1 font-bold text-md border-b p-4">
          <div className="size-2 rounded-full bg-chart-1 animate-pulse blur-[0.5px]" />
          AI ANALYSIS
        </CardTitle>
      </CardHeader>
      ...
      {isLoading ? (
        ...
        <CardContent>
          <div className="flex flex-col gap-5">
            <p className="text-[26px] font-bold leading-tight whitespace-pre-line">
              {article.summary}
            </p>
            ...
            >
              ...
              >
                <SentimentIcon
                  className={cn("w-5 h-5 mt-0.5", sentiment.color)}
                />
              </div>
              <div className="flex flex-col gap-2">
                <p
                  className={cn("text-sm font-bold uppercase", sentiment.color)}
                >
                  {data?.sentiment}
                </p>
      ...
      <CardFooter className="h-full flex flex-col gap-1 justify-end items-start w-full border-none">
        ...
        <div>
          <Badge className="uppercase py-3 px-3 bg-black  text-white border border-gray-600 tracking-wider rounded">
            {article.source}
          </Badge>
        </div>
      </CardFooter>
